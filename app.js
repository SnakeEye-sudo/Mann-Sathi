  ...
  (() => {
    ...
    const UI_COPY = {
      hi: {
        ...
        journal: "Journal",
        messages: "Messages",
        moodSummary: (value) => `Average mood ${value}`,
    ...
    const state = {
      ...
      mode: "dashboard",
      voice: localStorage.getItem(STORAGE.voice) || "male",
      entries: loadEntries(),
    ...
    function messagePool(type) {
      const sender = state.voice === "male" ? type === "morning" ? ["tumhari dost", "ek pyari dost", "soft bestie"] : ["tumhari dost", "soft bestie", "warm dost"] : type === "morning" ? ["tumhara dost", "ek caring dost", "steady bestie"] : ["tumhara dost", "steady bestie", "warm dost"];
      const openings = type === "morning" ? ["Good morning", "Nayi subah mubarak", "Aaj ka din ready hai", "Subah ka pyaara hello", "Utho champ", "Soft morning note"] : ["Good night", "Raat ka halka sa hug", "Aaj ka din ab rest le", "Night note", "Aaram se so jao", "Din ko pyaar se wrap karo"];
      const middles = type === "morning" ? ["tum aaj bhi sambhal loge", "aaj ka pace tumhare control me hai", "khud par thoda sa pyaar rakho", "small steps bhi kaafi hote hain", "aaj ka din tumhare saath hai", "dil ko halka rakhna"] : ["aaj jo bhi hua uska weight ab dheere se rakh do", "tumne jitna kiya woh kaafi tha", "kal fir se fresh start milega", "khud ko aaj ke liye maaf kar do", "rest bhi progress ka part hai", "ab dimag ko chain do"];
      const closings = type === "morning" ? ["main yahin hoon, tumhari ${friend}.", "aaj apne aap se softly milna.", "bas present rehna, perfect nahi.", "aaj ek achhi line zarur likhna.", "thoda sa muskura ke start karo.", "tumhara din accha jaa sakta hai."] : ["ab chain se so jana, tumhari ${friend}.", "kal ke liye hope sambhal ke rakhna.", "aaj ka chapter pyaar se close karo.", "raat tumhe reset de rahi hai.", "ab bas saans halka kar lo.", "sleep ko apna soft reset banne do."];
      const templates = [];
    ...
    function renderHero() {
      ...
      $("moodMeta").textContent = t("moodSummary")(averageMood().toFixed(1));
    }
    ...
    function renderEntries() {
      ...
      <div class="history-item">
        <strong>${entry.title || "Untitled"}</strong>
        <small>${entry.createdAt.slice(0, 10)} \xB7 mood ${entry.mood}</small>
      </div>
    `).join("");
    }
    ...
    function renderProfileSummary() {
      ...
      <div class="stack-row"><span>Cloud</span><strong>${state.authUser ? t("cloudUser") : t("cloudReady")}</strong></div>
    ...
    function saveEntry() {
      ...
      $("entryStatus").textContent = t("entrySaved");
    ...
    async function saveSchedule() {
      ...
      $("scheduleStatus").textContent = t("scheduleSaved");
      renderEverything();
      queueCloudSave();
    }
    async function maybeNotify(type) {
